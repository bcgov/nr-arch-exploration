...
oc create -n "$OPENSHIFT_NAMESPACE" configmap "$APP_NAME"-config-map --from-literal=TZ="$TZVALUE" --from-literal=OIDC_AUTH_SERVER_URL="$OIDC_AUTH_SERVER_URL" --from-literal=DB_HOST="$DB_HOST" --from-literal=DB_PORT="$DB_PORT" --from-literal=SERVICE_NAME="$SERVICE_NAME" --from-literal=FILE_LOG_LEVEL="$FILE_LOG_LEVEL" --from-literal=QUARKUS_DATASOURCE_JDBC_PASSWORD="$QUARKUS_DATASOURCE_JDBC_PASSWORD" --from-literal=QUARKUS_DATASOURCE_JDBC_USERNAME="$QUARKUS_DATASOURCE_JDBC_USERNAME" --from-literal=FILE_LOG_LEVEL="INFO" --dry-run -o yaml | oc apply -f -
echo
